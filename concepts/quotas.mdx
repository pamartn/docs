...
When your quota reaches 0, API requests return a `429 Too Many Requests` error
with `code: "QUOTA_EXHAUSTED"` (distinct from per-second `RATE_LIMITED`
throttling — branch on `code`):
