...
You'll receive this error in two scenarios. **Branch on the `code` field** to
tell them apart — the HTTP status is identical:

**Per-second limit exceeded** (`code: "RATE_LIMITED"`) — back off and retry,
honoring the `Retry-After` header (seconds):
```json
{
...
**Monthly quota exceeded** (`code: "QUOTA_EXHAUSTED"`) — upgrade your plan or
wait for renewal; retrying won't help until then:
```json
{
